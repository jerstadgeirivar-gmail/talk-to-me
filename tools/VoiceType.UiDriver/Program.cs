...
if (!settingsScenario && args.Length is not 3 and not 5)
{
    Console.Error.WriteLine(
        "Usage: VoiceType.UiDriver <application-path> <audio-fixture-path> <evidence-directory> [diagnostic-transcript|--live target-application-path]");
    return 2;
}
...
static async Task<int> MigrateCredentialAsync(string sourcePath)
{
    ...
    try
    {
        ...
        byte[] protectedBytes = await File.ReadAllBytesAsync(
            VoiceTypeDataPaths.ProtectedSecretFile,
